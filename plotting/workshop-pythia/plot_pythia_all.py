...
#memit_data = json.load(open('memit_results.test.final.json'))
...
for task in tasks:
    ...
    plt.ylabel('Task Success Rate', fontsize=20)
    plt.title(task.title() +' (0.0001 League)', fontsize=24)
...
plt.legend(fontsize=16)
plt.title('Average Success over Tasks (0.0001)', fontsize=24)
plt.xlabel('Model Size (Billions)', fontsize=20)
plt.ylabel('Task Success Rate', fontsize=20)
plt.tight_layout()
