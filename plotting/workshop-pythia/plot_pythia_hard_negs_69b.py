...
def get_latex_table():
  ...
  for task in tasks:
    ...
    prefix += '& {:.1f}'.format((average_initial[(models[0], task)]))
    for league in leagues:
      results = [(best_for_league_task[(models[0], task, league, method)] -initial_results[(models[0], task, league, method)]) for method in ('full', 'lora', 'memit')]
  ...
  prefix += '& {:.1f}'.format((avg))
  for league in leagues:
    prefix += ''.join(['& {:.1f}'.format(average_results[(league, method)]) for method in ('full', 'lora', 'memit')])
    prefix += '& '
  prefix += ' \\\\ \n'
  prefix += r'''
  \bottomrule
